...
def interpolate_time_linear(data: xr.DataArray, factor: int = 2) -> xr.DataArray:
    ...
    # Neue Zeitachse: gleichmäßig verteilte Zeitpunkte (im Unix-Zeitformat)
...
def preprocessing(ds, features: list, depths: list, dim: str, interpolate: int):
    """
...
def preprocessing_conv(ds, features: list, depths: list, interpolate: int):
    def standardize(group):
        m = group.mean("time")
        s = group.std("time")
        return (group - m) / s
...
def plot_average_loss_map_from_data(original_z, reconstructed_z, labels, cmin=None, cmax=None):
    ...
    fig.colorbar(im, cax=cbar_ax, orientation="horizontal", label="Loss (MSE)")
    fig.subplots_adjust(wspace=0.05, hspace=0.05, left=0, right=1, top=0.98, bottom=0.12)
    plt.show()
